...
// Load saved frequencies and dwell times when switching to Frequency Hop mode
function loadSavedFrequencies() {
    const savedFrequencies = JSON.parse(localStorage.getItem('savedFrequencies')) || [];
    const savedDwellTimes = JSON.parse(localStorage.getItem('savedDwellTimes')) || [];

    savedFrequencies.forEach((frequency, index) => {
        if (index < 10) {
            document.getElementById(`freq${index + 1}`).value = frequency;
            const dwellTime = savedDwellTimes[index] || 0;
